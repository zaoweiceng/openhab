...
import org.openhab.binding.mymiio.internal.transport.Communication;
...
public abstract class MiIoHandler extends BaseThingHandler implements MessageListener {
    ...
    protected  MyMiIOConfiguration configuration;
    ...
    public void initialize() {
        logger.info("Initializing Mi IO device handler '{}' with thingType {}", getThing().getUID(),
                getThing().getThingTypeUID());
        configuration = getConfigAs(MyMiIOConfiguration.class);
        ...
        if (pollingPeriod > 0) {
            pollingJob = scheduler.scheduleWithFixedDelay(() -> {
                try {
                    ...
                    logger.info("Unexpected error during refresh.", e);
                }

            }, 10, pollingPeriod, TimeUnit.SECONDS);
        } else {
            logger.info("Polling job disabled. for '{}'", getThing().getUID());
