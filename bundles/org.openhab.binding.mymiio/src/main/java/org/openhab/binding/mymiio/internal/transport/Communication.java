 ...
 */
package org.openhab.binding.mymiio.internal.transport;
...
import org.openhab.binding.mymiio.annotation.MessageListener;
...
import org.openhab.binding.mymiio.internal.MiIoCrypto;
import org.openhab.binding.mymiio.internal.MyMiIOBindingConstants;
...
public class Communication {
    ...
    MiIoSendCommand sendMiIosendCommand(MiIoSendCommand miIoSendCommand) {
        ...
        try {
            ...
            if(res.isJsonObject()){
                needPing = false;
                miIoSendCommand.setResponse(res.getAsJsonObject());
    ...
    public Message sendPing(String ip) throws IOException {
        for (int i = 0; i < 3; i++) {
            Message resp = sendData(MyMiIOBindingConstants.DISCOVER_STRING, ip);
            if (resp != null){
                pingSuccess();
                return resp;
            }
    ...
    private synchronized byte[] comms(byte[] msg, String ip) throws IOException {
        ...
        try{
            ...
            sendData = msg;
            DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, ipAddress, MyMiIOBindingConstants.PORT);
