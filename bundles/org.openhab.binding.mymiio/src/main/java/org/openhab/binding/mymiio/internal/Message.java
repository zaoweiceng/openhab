...
public class Message {
    ...
    public Message(byte[] raw){
        this.raw = Arrays.copyOf(raw, (raw.length < 32) ? 32 : raw.length);
        this.header = Arrays.copyOf(this.raw, 16);
        this.magic = Arrays.copyOf(this.raw, 2);
        byte[] msgL = Arrays.copyOfRange(this.raw, 2, 4);
        this.length = ByteBuffer.wrap(msgL).getShort();
        this.unknowns = Arrays.copyOfRange(this.raw, 4, 8);
        this.deviceId = Arrays.copyOfRange(this.raw, 8, 12);
        this.tsByte = Arrays.copyOfRange(this.raw, 12, 16);
        this.checksum = Arrays.copyOfRange(this.raw, 16, 32);
        this.data = Arrays.copyOfRange(this.raw, 32, length);
    }
